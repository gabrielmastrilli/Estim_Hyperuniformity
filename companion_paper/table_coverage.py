...
### Coverage of alpha_hat for perturbed lattices using the non asymptotic covariance matrix.
...
for i_alpha in range(len(alphas)):
    ...
    for i_R in range(len(Rs)):
        ...
        print(alpha, R)
        alpha_hats = np.loadtxt("./companion_paper/data/estim_perturbed_lattices/alpha_"+str(alpha)+"_R_"+str(R)+".txt")
        #Compute the quantiles of order q_1 = 0.025 and q_2 = 0.975 of log(R)(alpha_hat - alpha).
        q_1, q_2 =  confidence_interval.quantile_non_asymptotic(0.025, 0.975, R, alpha, J, i_min, i_max)
        print(q_1, q_2)
        n_inside = 0
...
np.savetxt("./companion_paper/data/coverage/nb_inside.txt", p_insides)
print(p_inside)

### Coverage of alpha_hat for perturbed lattices using the asymptotic covariance matrix.
...
for i_alpha in range(len(alphas)):
    ...
    for i_R in range(len(Rs)):
        ...
        #Compute the quantiles of order q_1 = 0.025 and q_2 = 0.975 of log(R)(alpha_hat - alpha).
        alpha_hats = np.loadtxt("./companion_paper/data/estim_perturbed_lattices/alpha_"+str(alpha)+"_R_"+str(R)+".txt")
        q_1, q_2 =  confidence_interval.quantile_asymptotic(0.025, 0.975, R, alpha, J, i_min, i_max)
        print(q_1, q_2)
        n_inside = 0
        for alpha_hat in alpha_hats:
            if alpha_hat > alpha + q_1 and alpha_hat < alpha+q_2:
        ...
        print(p_inside)
        p_insides[i_alpha, i_R]= p_inside
#np.savetxt("./companion_paper/data/coverage/nb_inside_ind.txt", p_insides)
print(p_inside)
